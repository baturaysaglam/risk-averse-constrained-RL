...
class TimeLimitMask(gym.Wrapper):
    def step(self, action):
        ...
        is_gymnasium = len(stepped) == 6
        if is_gymnasium:
            obs, rew, cost, terminated, truncated, info = stepped
            done = terminated or truncated
        else:
            obs, rew, done, info = stepped
...
class TransposeObs(gym.ObservationWrapper):
    def __init__(self, env=None):
        """
        Transpose observation space (solver class)
        """
...
class VecPyTorch(VecEnvWrapper):
    ...
    def step_wait(self):
        stepped = self.venv.step_wait()
        is_gymnasium = len(stepped) == 5
        if is_gymnasium:
            obs, reward, cost, done, info = stepped
        else:
